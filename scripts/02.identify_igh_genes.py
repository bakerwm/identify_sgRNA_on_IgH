...
"""
Script to identify IgH genes in the mouse genome by searching gene_name for:
IGH*, Igh*, immunoglobulin, heavy

IgH genes are typically located on chromosome 12.
with the following coordinates:
chr12:113,254,829-116,010,093 (GRCm38/mm10)
chr12:113,258,768-116,009,954 (GRCm39/mm39)
...
def identify_igh_genes(gtf_file, output_file, chr_list=['12']):
    """
    ...
    with open(gtf_file, 'r') as f:
        for line in f:
            ...
            if (gene_name.startswith('IGH') or 
                gene_name.startswith('Igh') or  # Added to catch mouse gene naming
    ...
    if len(igh_genes) == 0:
        ...
        # Mouse IgH locus is located on chromosome 12, approximately at 113,258,768-116,009,954 in mm10
        with open(output_file, 'w') as out:
...
def merge_regions(regions_file, output_file, max_distance = 1000000):
    ...
    """
    # Using pybedtools
    pybedtools.BedTool(regions_file).merge(d=max_distance, c='4,5,6', o='first,first,first').saveas(output_file)        

def main():
    parser = argparse.ArgumentParser(description="Identify IgH genes in mouse genome")
    ...
    parser.add_argument('--chr_list', required=False, default=['12'], help="List of chromosomes to search for IgH gene, default: 12")
